...
void draw() {
  ...
  for (int y = 0; y < rows-1; y++) {
    ...
    for (int x = 0; x < cols; x++) {
      ...
      if (TerrainColor) {
        if (z1 > zScale/3) fill(255);
        else if (z1 > -zScale/3) fill(50, 120, 30);
        else if (z1 > -zScale/2) fill(200, 140, 40);
        else fill(90, 150, 230, 200);
      }
      vertex(x*scale, y*scale, z1); // draw vertex

      float z2 = Terrain[x][y+1]; // calculate second vertex z-value and set color options
      if (TerrainColor) {
        if (z2 > zScale/3) fill(255);
        else if (z2 > -zScale/3) fill(50, 120, 30);
        else if (z2 > -zScale/2) fill(200, 140, 40);
        else fill(90, 150, 230, 200);
      }
